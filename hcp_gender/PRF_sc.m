...
Ts_10 = 0.1;
ind_BOLD_10 = use(sc).ind_BOLD_10;
...
rrf = r_PRF_sc(3)*100;
prf = r_PRF_sc(1)*100;
curve_feature = NaN(1,14);
[cpks,clocs,cwidth,~] = findpeaks(abs(CRF_sc));
if length(cpks) < 2
    return
end
if length(cpks) > 2
...
[rpks,rlocs,rwidth, ~] = findpeaks(abs(RRF_sc));
if length(rpks) < 2 
    return
end
...
curve_feature = [cpks(1:2) clocs(1:2)./10 cwidth(1)./10 cwidth(2)./10 c_shape rpks(1:2) rlocs(1:2)./10 rwidth(1)./10 rwidth(2)./10 r_shape];
